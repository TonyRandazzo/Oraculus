...
var attack_hit_time: float = 0.3  # Time in seconds when attack hits
...
var friendly_responses = [
	"*Sighing* Maybe you're not as useless as other humans...",
...
func setup_ai_profile():
	npc_ai._selected_character = {
		"name": "Demon",
		"description": "Concise and theatrical demon mage who can become a friend",
		"voice_ids": ["male"]
	}
...
func handle_attack_behavior(delta: float):
	if player and is_instance_valid(player):
		var dir = sign(player.global_position.x - global_position.x)
		velocity.x = dir * speed * (1.0 + current_aggression) # Faster when more aggressive
		sprite.flip_h = dir < 0
		
		# If player moves too far away, return to conversing state
		...
		if not is_attacking:
			...
			if distance < attack_range and can_attack:
				...
				# Tactical behavior: approach while maintaining distance
...
func start_attack():
	attack_timer = attack_cooldown * (1.5 - current_aggression * 0.5) # Shorter cooldown when aggressive
	can_attack = false
	is_attacking = true
	state = "attacking"
...
func _on_attack_box_area_entered(area: Area2D):
	if area.is_in_group("hit_player"):
		...
		if player.has_method("take_damage"):
			player.take_damage(attack_damage * (1.0 + current_aggression * 0.5)) # More damage when aggressive
...
func take_damage(amount: int):
	...
	# More aggressive reaction when hit
	if friendship_level < 3:
		state = "attacking"
		can_attack = true
		attack_timer = 0.0
...
func update_mood():
	...
	new_mood = mood_score.keys()[mood_score.values().find(mood_score.values().max())]
	intensity = mood_score[new_mood]
	
	if new_mood != current_mood:
		mood_intensity = lerp(mood_intensity, intensity, 0.3)
		if mood_intensity < 0.1:
...
func say_launch_message():
	var request_data = {
		...
		"stimuli": "Announce your presence with a threatening phrase"
	}
...
func send_structured_request(data: Dictionary):
	...
	if npc_ai:
		...
		var context = {
			...
			"health": float(current_health) / float(max_health),
			"request": data
		}
		
		timer.stop()
		npc_ai.notify(JSON.stringify(context))  # Send context to AI
