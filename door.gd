...
var possible_answers = ["Ulysses", "Bomi", "Hope"]
...
func _on_instructions_failed(error_code: int):
	...
	# Fallback with predefined riddles
	match correct_answer:
		"Ulysses":
			current_instructions = """I sailed seas and unknown lands,
Fought cyclops and sirens so grand,
My name is tied to a journey long,
Who am I?"""
		"Bomi":
			current_instructions = """Small but full of might,
In tunnel's dark I make my sound,
My name starts with B and ends with i,
Who am I?"""
		"Hope":
			current_instructions = """Last to leave the jar so wide,
Strongest when all else has died,
Men seek me in their darkest night,
Who am I?"""
		_:
			current_instructions = """The entity cannot formulate the riddle...
